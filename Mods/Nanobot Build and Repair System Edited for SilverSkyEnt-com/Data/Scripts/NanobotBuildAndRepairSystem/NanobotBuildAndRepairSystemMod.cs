namespace SpaceEquipmentLtd.NanobotBuildAndRepairSystem
{
   ...
   {
      private const string Version = "V1.6.24 2018-07-20";
      private const int MaxBackgroundTasks = 4;
      ...
      private const string CmdHelpText_Client = "Version: " + Version +
         "\nAvailable commands:" +
      ...
      private const string CmdHelpText_Server = CmdHelpText_Client +
         ...
         "\n[" + CmdCpsf + "]: Creates a global settings file inside mod folder (including all options).";
      ...
      protected override void UnloadData()
      {
         ...
         {
            Mod.Log.Error("NanobotBuildAndRepairSystemMod.UnloadData: Phase{0}, {1}", phase, e.ToString());
         }
         base.UnloadData();
      }
                  ...
                  if (attackerId != 0)
                  {
                     ...
                     foreach (var entry in BuildAndRepairSystems)
                     {
                        ...
                        if (MyRelationsBetweenPlayerAndBlockExtensions.IsFriendly(relation))
                        {
                           ...
                           //I don't check block relation here, because if it is owned by an enemy we won't repair it in any case and it just times out
      ...
      public static void AddAsyncAction(Action newAction)
      {
         lock (AsynActions)
         {
            ...
            if (ActualBackgroundTaskCount < MaxBackgroundTasks)
            {
               ActualBackgroundTaskCount++;
               if (Mod.Log.ShouldLog(Logging.Level.Info)) Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemMod: AddAsyncAction Start Task {0} of max {1}", ActualBackgroundTaskCount, MaxBackgroundTasks);
               MyAPIGateway.Parallel.StartBackground(() =>
               {
                  try
                  {
                     ...
                     if (Mod.Log.ShouldLog(Logging.Level.Info)) Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemMod: AddAsyncAction Task ended. Still running {0} of max {1}", ActualBackgroundTaskCount, MaxBackgroundTasks);
                  }
                  catch {
                     lock (AsynActions)
                     {
